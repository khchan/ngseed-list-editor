...
.directive('listEditor', function() {
    return {
        ...
        controller: function($scope, $element, $filter, ngTableParams, $sce) {
            ...
            var affixMissingColumns = function(listInput, colInput) {
                ...
                .map(function(m) {
                    missingCols.push({
                        field: m,
                        type: _.every(_.compact(_.pluck(listInput, m)), _.isNumber) ? 'number' : 'string'
                    });
                });
            ...
            $scope.$watchCollection('rowBuffer', function(newVal) {
                ...
                $scope.canAddRow = _.compact(_.values(newVal)).length == columns.length;
            });
            ...
            $scope.$watchCollection('columnBuffer', function(newVal) {
                ...
                $scope.canAddCol = _.compact(_.values(newVal)).length == columns.length;
            });
